    ...
    if(simu){
      ...
      if (millis() - gestiondelai >= delaims) {
        if (jauge1.wait_ready_timeout(1000) && jauge2.wait_ready_timeout(1000)) {
          digitalWrite(LEDROUGE_PIN, HIGH);
          float JPortance=round(jauge1.get_units(10) * 10.0) / 10.0;   // JTrainee = jauge trainée , JPortance = jauge portance
          float JTrainee=round(jauge2.get_units(10) * 10.0) / 10.0;  // Arrondi les valeurs des jauges de portance et de trainée 
          String mesure = String("PORTANCE " ) + abs(JPortance) + String(" TRAINEE ") + abs(JTrainee); // string final a envoyer a l'utilisateur
          Serial2.println(mesure);
          int traineeAnalog = abs(JTrainee) * 251 / eqgvolttrai; // 251 correspondant a 10V dans le système avec notre ampli OP afin d'obtenir Vmax = 10 V
          int portanceAnalog = abs(JPortance)  * 251 / eqgvoltport; // car si l'on prenait 255 vu que AmpliOP imparfait Vmax = 10.65V...
